...
Trả về JSON theo format sau, không có text nào khác:
{
  "scores": { "leadTime": <số>, "revenuePotential": <số>, "velocity": <số>, "crossPlatform": <số>, "feasibility": <số>, "saturation": <số> },
  "totalScore": <tổng>,
  "vibe": "<nhận xét ngắn 1-2 câu tiếng Việt về trend này>",
  "aiSuggest": "<gợi ý sản xuất cụ thể: BPM, mood, style, phù hợp dùng ở đâu>"
}`
...
export async function runScorer() {
  ...
  try {
    ...
    for (const result of results) {
      ...
      const urgency = getUrgency(result.totalScore)
